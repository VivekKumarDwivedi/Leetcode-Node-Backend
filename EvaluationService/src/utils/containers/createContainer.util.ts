...
export async function createNewDockerContainer(options: CreateContainerOptions){
         try{
            ...
            const container = await docker.createContainer({
                ...
                AttachStderr: true,
                Tty: false,
                OpenStdin: true,// not all code need input 
                HostConfig:{
                    Memory: options.memoryLimit,
                    PidsLimit: 100, // to limit the number of processes
                    CpuQuota: 50000, // to limit the CPU usage\
                    CpuPeriod: 100000, // to limit the CPU usage
            ...
            logger.error("Error creating new container",error);
            return null;
         }
}