...
export class ProblemService implements IProblemService {
    ...
    constructor(problemRepository: IProblemRepository) { // constuctor based dependency injection
    ...
    async getProblemById(id: string): Promise<IProblem | null> {
        ...
        if(!problem) {
            throw new NotFoundError("Problem is not found");
        }
        return problem;
    }
    ...
    async updateProblem(id: string, updateData: UpdateProblemDto): Promise<IProblem | null> {
        ...
        if(updateData.editorial){
            editorial: updateData.editorial && await sanitizeMarkdown(updateData.editorial)
        }
